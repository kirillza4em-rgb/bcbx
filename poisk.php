...
<head>
   ...
   <script>
    ...
    function checkForNewRequests() {
        fetch(`check_new_requests.php?last_time=${encodeURIComponent(lastCheckTime)}`)
            .then(response => response.json())
            .then(data => {
                if (data.new_count > 0) {
                    let message = `Появились новые заявки: ${data.new_count}\n`;
                    
                    // Проходим по массиву новых заявок и добавляем информацию в сообщение
                    data.new_requests.forEach(request => {
                        message += `ФИО: ${request.FIO}, Отдел: ${request.otdel}, Категория: ${request.categoriy}, Приоритет: ${request.prioritet}\n`;
                    });
                    
                    // Отображаем уведомление с перечислением новых заявок
                    alert(message);

                    // Обновляем lastCheckTime на текущее время
    ...
    // Проверяем новые заявки каждые 30 секунд
